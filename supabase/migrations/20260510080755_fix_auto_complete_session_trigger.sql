-- Fix auto_complete_session trigger: quoted string literal for status column (was unquoted identifier)
...
BEGIN
  ...
  IF NEW.ended_at IS NOT NULL AND OLD.ended_at IS NULL THEN
    ...
    ELSE
      ...
      _teacher_base := ROUND((_duration_minutes::numeric / 45.0) * 20.0, 2);
      _platform_fee := ROUND((_duration_minutes::numeric / 45.0) * 40.0, 2);
      _net          := _teacher_base;
      ...
      NEW.platform_fee_rate_snapshot := ROUND(40.0 / 60.0, 6); NEW.net_amount := _net;
      ...
      IF _sub_id IS NOT NULL THEN
        UPDATE user_subscriptions SET remaining_minutes = GREATEST(0, remaining_minutes - _duration_minutes) WHERE id = _sub_id;
      END IF;
      UPDATE bookings SET status = 'completed' WHERE id = NEW.booking_id;
    END IF;
  END IF;

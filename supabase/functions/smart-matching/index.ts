...
serve(async (req) => {
  ...
  const rate = checkEdgeRateLimit(req, "smart-matching", 20);
  ...
  try {
    ...
    if (!token) return new Response(JSON.stringify({ error: "غير مصرح" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });

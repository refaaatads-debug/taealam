...
serve(async (req) => {
  ...
  const rate = checkEdgeRateLimit(req, "session-report", 10);
...
المخالفات: ${totalViolations}`;
    ...
    const systemPrompt = `أنت محلل تعليمي صارم. مهمتك الوحيدة: وصف ما دار **فعلياً** داخل هذه الجلسة بين المعلم والطالب، بناءً على Timeline المرفق فقط.

⛔ قواعد قطعية لا تُكسر أبداً (أي خرق = تقرير مرفوض):
1. **ممنوع منعاً باتاً** ذكر أي معلومة، نصيحة، شرح، مثال، أو موضوع لم يَرِد حرفياً في Timeline.
2. **ممنوع** الخروج عن إطار الجلسة: لا معلومات عامة عن المادة، لا نصائح دراسية عامة، لا مقارنات خارجية، لا اقتراح موارد، لا "من المعروف أن..."، لا "عادةً ما..."، لا تخمينات.
3. **ممنوع** إضافة أي رأي شخصي أو تفسير نفسي/تربوي غير موثّق باقتباس حرفي من المحادثة.
4. كل جملة في التقرير **يجب** أن تكون إما:
   (أ) رقم/إحصائية مأخوذة حرفياً من البيانات المرفقة، أو
   (ب) اقتباس مباشر بين علامتي تنصيص "..." من رسالة فعلية، مع ذكر [MM:SS] والمتحدث.
5. إذا كان Timeline فارغاً أو ضعيفاً في جانب ما، اكتب حرفياً: "لا تتوفر بيانات كافية لهذا الجانب" — ولا تخترع شيئاً.
6. ممنوع كتابة توصيات عامة. التوصيات الوحيدة المسموح بها: ربط مباشر بحدث حدث في الجلسة (مثال: "في [05:23] طرح الطالب سؤالاً ولم يتلقَّ إجابة — يُنصح بمعالجة هذا السؤال").
7. لا تذكر أي اسم شخص، مكان، تطبيق، أو رابط لم يُذكر في الرسائل.

✍️ الهيكل الإلزامي (التزم به حرفياً):

📋 ما دار في الجلسة (سرد زمني حصري لما حدث):
- اذكر الأحداث بالترتيب مع [MM:SS] واقتباسات حرفية بين "..."
- لا تضف أي تفسير خارج الاقتباس

📊 إحصائيات فعلية:
- شروحات: ${timelineStats.explanations_count} | أسئلة: ${timelineStats.questions_count} | إجابات: ${timelineStats.answers_count}
- مدة كلام المعلم: ${rawStats.teacher_speaking_minutes} د | الطالب: ${rawStats.student_speaking_minutes} د
- فترات صمت: ${timelineStats.silence_events}

🔍 المواضيع التي ذُكرت فعلياً (من نصوص الرسائل فقط):
- اقتبس الجملة التي يظهر فيها الموضوع

⚠️ ملاحظات مرتبطة بأحداث الجلسة فقط:
- اذكر فقط الفجوات الموثّقة باقتباس + وقت

🎯 تقييم الأداء: ${performanceScore}/100 (محسوب آلياً من بيانات الجلسة)

🚫 لا تكتب قسم "توصيات عامة". إن لم تجد توصية مرتبطة بحدث محدد في الجلسة، احذف هذا القسم.`;
    ...
    return new Response(JSON.stringify({ error: "تعذر إنشاء تقرير الحصة" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});

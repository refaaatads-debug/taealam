...
export default function CallStudentButton({
  ...
  useEffect(() => {
    if (!chooserOpen || !bookingId) return;
    setLoading(true);
    (async () => {
      ...
      if (booking?.student_id) {
        ...
        const [{ data: privateProfile }, { data: publicProfile }] = await Promise.all([
          supabase.from("profiles").select("phone").eq("user_id", booking.student_id).maybeSingle(),
          supabase.from("public_profiles").select("full_name").eq("user_id", booking.student_id).maybeSingle(),
        ]);
        setStudentPhone(privateProfile?.phone || "");
        setResolvedName(studentName || publicProfile?.full_name || "الطالب");
      }
    })().finally(() => setLoading(false));
  }, [chooserOpen, bookingId, studentName]);
          ...
          {loading ? (
            ...
            <div className="grid gap-3 py-3 sm:grid-cols-2">
              <button
                ...
                className="group rounded-2xl border-2 border-emerald-200 bg-emerald-50/60 p-5 text-right transition hover:border-emerald-400 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                ...
                <p className="mt-2 text-xs leading-5 text-slate-600">يرن لدى الطالب ويمكنه القبول أو الرفض. يُغلق تلقائيًا بعد دقيقتين.</p>
